...
def make_layers5x5(cfg, batch_norm=False):
    layers = []
    ...
    for l in cfg:
        ...
        layers += [nn.Conv2d(input_channel, l, kernel_size=5, padding=2)]
...
def plain_net5(num_class=10):
    return plain_net(make_layers(cfg['G'], batch_norm=True), num_class=num_class)



def plain_net5_5x5(num_class=10):
    return plain_net(make_layers5x5(cfg['G'], batch_norm=True), num_class=num_class)


def plain_net6_diff(num_class=10):
...
def plain_net5_leaky_relu(num_class=10):
